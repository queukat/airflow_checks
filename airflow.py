...
write_to_hive = HiveOperator(
    ...
    hql=';\n'.join(
        '{{ task_instance.xcom_pull(task_ids="write_tasks_to_hive", key="hive_tasks")[{}] }}'.format(i) for i in
        range(len('{{ ti.xcom_pull(task_ids="write_tasks_to_hive", key="hive_tasks") }}'))),
    dag=dag,
)
